...
@ProfilePage.route("/AdminProfile/Department",methods=["POST","GET"])
def AdminDepartment():
    if session["Group_id"]=="A":
        ...
        return render_template("ShowDataToAdmin.html",Type='Department',Data=Result)
    else :
        return redirect("/error")
###########################################################################################

@ProfilePage.route("/AdminProfile/Employee",methods=["POST","GET"])
def AdminEmployee():
    if session["Group_id"]=="A":
        #query to get the data of the employees
        Result= selectEmployeesExceptAdmin(cursor)
        return render_template("ShowDataToAdmin.html",Type="Employee",Data=Result)
        
    else:
        return redirect("/error")
###########################################################################

@ProfilePage.route("/AdminProfile/Patient",methods=["POST","GET"])
def AdminPatient():
    if session["Group_id"]=="A":
        Result= selectFromTable(cursor,'Patient',Patient_attributes,[Patient.All.value] ,[] )
        return render_template("ShowDataToAdmin.html",Type='Patient',Data=Result)
    else: 
        return redirect("/error")

#############################################################################################

@ProfilePage.route("/AdminProfile/Rooms",methods=["POST","GET"])
def AdminRoom():
    if session["Group_id"]=="A":
        Result= selectFromTable(cursor,'Room',Patient_attributes,[Room.All.value] ,[] )
        return render_template("ShowDataToAdmin.html",Type='Rooms',Data=Result)
    else: 
        return redirect("/error")

@ProfilePage.route("/AdminProfile/Donation",methods=["POST","GET"])
def AdminDonation():
    if session["Group_id"]=="A":
        Result=selectFromTable(cursor,'Donation',Donation_attributes,[Donation.All.value] ,[] )
        return render_template("ShowDataToAdmin.html",Type='Donation',Data=Result)
    else:
        return redirect("/error")

@ProfilePage.route("/AdminProfile/Dashboard",methods=["POST","GET"])
def AdminDashboard():
    if session["Group_id"]=="A":
        ...
        Patients.pop(-1)            
        return render_template("statistics.html",Type="Dashboard",Data1=numbers,number1=len(numbers)-1,Depart1=DepartmentsofDoctors,Data2=numbersOfPatients,number2=len(numbersOfPatients)-1,Doct=Doctors,Data3=Patients,number3=len(Patients)-1,Depart2=DepartmentsOfPatients)
        
    else:
        return redirect("/error")  #"you are not allowed"

#route to insert the Departments by the Admin
@ProfilePage.route("/AdminProfile/Departments/Insert",methods=["POST","GET"])
def AdminSettingsDepartments():
    if session["Group_id"]=="A":
        info = cursor.execute('''select ID from Employee ;''').fetchall()
        return render_template("AdminSettings.html",SSNList=info[0],Type="Departments",selector="Insert")
    else:
        return redirect("/error")  #"you are not allowed"
#route to insert the employees by the Admin
@ProfilePage.route("/AdminProfile/Employees/Insert",methods=["POST","GET"])
def AdminSettingsEmployees():
    if session["Group_id"]=="A":
        Departments=cursor.execute('''select ID from Department ;''').fetchall()
        return render_template("AdminSettings.html",SSNList=session,DEList=Departments[0],Type="Employees",selector="Insert")
    else:
        return redirect("/error")  #"you are not allowed"
#########################DONE#########################################################

@ProfilePage.route("/AdminProfile/Rooms/Insert",methods=["POST","GET"])
def AdminSettingsRooms():
    if session["Group_id"]=="A":
        Departments=cursor.execute('''select ID from Department ;''').fetchall()
        return render_template("AdminSettings.html",SSNList=session,DEList=Departments[0],Type="Rooms",selector="Insert")
    else:
        return redirect("/error")  #"you are not allowed"
#########################DONE#########################################################
#rout to check the data form the Admin
@ProfilePage.route("/AdminProfile/checkdata/Department/Insert",methods=["POST"])
def AdmincheckoutDataforInsertDepartment():
    if request.method=="POST":
...
############################################################################################################################
@ProfilePage.route("/AdminProfile/checkdata/Employee/Insert",methods=["POST"])
def AdmincheckoutDataforInsertEmployee():
    if request.method=="POST" :
        ...
        return redirect("/error")
########################################################################################################
@ProfilePage.route("/AdminProfile/checkdata/Room/Insert",methods=["POST"])
def AdmincheckoutDataforInsertRoom():
    if request.method=="POST" :
        # the data of the employee
        state=request.form.get("state")
        lastdate=request.form.get("laststay")
        Department=request.form.get("Departmentid")
        if state != "" and lastdate !="" and Department!="" :
            new_info=[lastdate,state,Department]
            print("mahoud")
            print(new_info)
        else :
            return redirect("/error")  #'wrong data'           
        insert_general(cursor,'Room',Room_attributes,[Room.All.value],new_info)
        connection.commit()
        return redirect("/AdminProfile/Rooms")
    else:
        return redirect("/error")
########################################################################################################
@ProfilePage.route("/AdminProfile/checkdata/Edit/Department/<int:ID>",methods=["POST"])
def AdmincheckoutDataforEditDepartment(ID):
    if request.method== "POST" :
        #the data of the department
        Departmentname=request.form.get("Departmentname")
        Startdate=request.form.get("StartDate")
...
@ProfilePage.route("/AdminProfile/checkdata/Edit/Employee/<int:ID>",methods=["POST"])
def AdmincheckoutDataforEditEmployee(ID):
    if request.method== "POST" :
        # the data of the employee
...
##########################################################
##################DONE#############################
# route to delete the rooms by using the id
@ProfilePage.route('/DeleteRoom/<int:id>')
def deleteRoom(id):
    # query to delete
    Delete(cursor,'Room',[Room.Room_ID.value],[id])
    connection.commit()
    return redirect("/AdminProfile/Rooms")
##########################################################
...
########################################################################################################################
@ProfilePage.route('/EditRoom/<int:id>')
def EditRoom(id):
    # query to retrive the data of the id
    department_Data = selectFromTable(cursor,'Department',Department_attributes,[Deparment.All.value] ,[(Deparment.Department_ID.value,id)] )
    manager_Data = selectFromTable(cursor,'Employee',Employee_attributes,[Employee.Employee_ID.value],[])
...
def showAllPatient(employee_id):
    patients = [patient[1] for patient in select_patients_reservation_doctor(cursor, session['ID'])]
...
@ProfilePage.route("/insertprescripation/<int:id>",methods=["POST"])
def insertprescripation(id):
    illness = request.form.get("illness")
    treatment = request.form.get("treatment")

    reservation = select_reservation(cursor,session["ID"], id)
