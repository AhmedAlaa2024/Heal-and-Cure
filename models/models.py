...
def activate_Fks(cursor):
    ...
    query2 = " PRAGMA case_sensitive_like = true;"
    try:
...
def count_doctors_Departments(cursor):
    query='''select D.Department_Name, count(*) as counter from Employee E,Department D where E.D_id = D.ID  group by D.Department_Name;'''
    cursor.execute(query)
    result=cursor.fetchall()
    return result

#----------- number of patients visited each department

def count_patient_Doctor(cursor):
    query='''select R.Doctor_id,E.Fname, count(*) as counter from Employee E,Patient P,Reservation R where E.ID = R.Doctor_id and P.ID = R.Patient_id  group by R.Doctor_id,E.Fname;'''
    cursor.execute(query)
    result=cursor.fetchall()
    return result

#----------- number of patients visited in each department
def count_patients_Doctors_department(cursor):
    query='''select D.Department_Name,count(*) as counter from Department D,Employee E,Patient P,Reservation R where D.ID = E.D_id and E.ID = R.Doctor_id and P.ID = R.Patient_id  group by D.Department_Name;'''
    cursor.execute(query)
    result=cursor.fetchall()
    return result